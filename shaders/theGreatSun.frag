...
//colorUp.b   = fract(0.5 + (sin((u_time * timeScale) * PI) * 0.5));
//colorDown.b = fract(0.5 + (cos((u_time * timeScale) * PI) * 0.5));

float Circle(float radius, vec2 uv, vec2 position){
    return 1.0 - step(radius, length(uv - position));
}

float SmoothCircle(float radius, vec2 uv, vec2 position, float edgeWidth){
...
void main(){
    ...
    color *= Circle(0.5, uv, vec2(0.5, 0.5));
    float edgeMask = (1. - SmoothCircle(0.49, uv, vec2(0.5, 0.5), 0.01)) * SmoothCircle(0.5, uv, vec2(0.5, 0.5), 0.01);
